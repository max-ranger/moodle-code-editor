...
/**
 * Renderer class for the code question type.
 *
 * @package    qtype
 * @subpackage code
...
class qtype_code_renderer extends qtype_renderer {
    public function formulation_and_controls(question_attempt $qa,
        ...
        if (empty($options->readonly)) {
            ...
            $answer = $responseoutput->response_area_input('answer', $qa,
                    $step, $question->responsefieldlines, $options, $options->context);
    ...
    public function files_read_only(question_attempt $qa, question_display_options $options) {
        ...
        $output = html_writer::tag('h4', get_string('answerfiles', 'qtype_code'), ['id' => $labelbyid, 'class' => 'sr-only']);
    ...
    public function files_input(question_attempt $qa, $numallowed,
        ...
        if (!empty($qa->get_question()->filetypeslist)) {
            $text = html_writer::tag('p', get_string('acceptedfiletypes', 'qtype_code'));
        ...
        $output .= html_writer::tag('legend', get_string('answerfiles', 'qtype_code'), ['class' => 'sr-only']);
    ...
    public function manual_comment(question_attempt $qa, question_display_options $options) {
        ...
        return html_writer::nonempty_tag('div', $question->format_text(
                $question->graderinfo, $question->graderinfoformat, $qa, 'qtype_code',
...
abstract class qtype_code_format_renderer_base extends plugin_renderer_base {
    /**
     * Render the students respone when the question is input mode.
    ...
    abstract public function response_area_input($name, question_attempt $qa,
                                                 question_attempt_step $step, $lines, $options, $context);
...
/**
 * An code format renderer for codes where the student should use the monaco
 * editor without the file picker.
 *
 * @copyright  2022 Max Ranger
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class qtype_code_format_editor_renderer extends plugin_renderer_base {
    protected function class_name() {
        return 'qtype_code_editor';
    }

    public function response_area_input($name, $qa, $step, $lines, $options, $context) {
        global $CFG;
        ...
        $editor->use_editor($id, $this->get_editor_options($qa->get_question(), $options, $context),
                $this->get_filepicker_options($context, $draftitemid));

        $output = html_writer::tag('label', get_string('answertext', 'qtype_code'), [
            'class' => 'sr-only',
            'for' => $id,
        ]);
        $output .= html_writer::start_tag('div', array('class' =>
                $this->class_name() . ' qtype_code_response'));
     ...
     */
    protected function get_editor_options(qtype_code_question $question, $options, $context) {
        // Disable the text-editor autosave because quiz has it's own auto save function.
        return array('language' => $question->responseformat,
...
/**
 * An code format renderer for codes where the student should use the HTML
 * editor with the file picker.
 *
 * @copyright  2011 The Open University
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class qtype_code_format_editorfilepicker_renderer extends qtype_code_format_editor_renderer {
    protected function class_name() {
        return 'qtype_code_editorfilepicker';
    }
     ...
     */
    protected function get_editor_options(qtype_code_question $question, $options, $context) {
        $fpoptions = question_utils::get_editor_options($context);
        // Disable the text-editor autosave because quiz has it's own auto save function.
        return array_merge($fpoptions, array('language' => $question->responseformat, 'readOnly' => $options->readonly));
    }
    ...
    protected function specific_filepicker_options($acceptedtypes, $draftitemid, $context) {
        debugging('qtype_code_format_editorfilepicker_renderer::specific_filepicker_options() is deprecated, ' .
