...
/**
 * code question type editing form.
 */
class qtype_code_edit_form extends question_edit_form {

    protected function definition_inner($mform) {
        global $CFG;
        MoodleQuickForm::registerElementType('codeeditor',
                                            "$CFG->dirroot/lib/editor/monaco/codeeditorelement.php",
                                            'MoodleQuickForm_codeeditor');

        $qtype = question_bank::get_qtype('code');

        if (!isset($this->question->options) || is_null($this->question->options) ||
            !isset($this->question->options) || is_null($this->question->options->responseformat)) {
            $codelanguage = $this->get_default_value('responseformat', 'csharp');
        } else {
            $codelanguage = $this->question->options->responseformat;
        }
        ...
        $mform->addElement('select', 'responseformat',
                get_string('responseformat', 'qtype_code'), $qtype->response_formats(),
                ['onchange' => "this.form.elements['id_updatebutton'].click();"]);
        $mform->setDefault('responseformat', $this->get_default_value('responseformat', 'csharp'));

        $mform->addElement('select', 'responserequired',
                get_string('responserequired', 'qtype_code'), $qtype->response_required_options());
        ...
        $mform->addElement('select', 'responsefieldlines',
                get_string('responsefieldlines', 'qtype_code'), $qtype->response_sizes());
        ...
        $mform->addGroup($mingrp, 'mingroup', get_string('minwordlimit', 'qtype_code'), ' ', false);
        $mform->addHelpButton('mingroup', 'minwordlimit', 'qtype_code');
        ...
        $mform->addGroup($maxgrp, 'maxgroup', get_string('maxwordlimit', 'qtype_code'), ' ', false);
        $mform->addHelpButton('maxgroup', 'maxwordlimit', 'qtype_code');
        ...
        $mform->addElement('select', 'attachments',
                get_string('allowattachments', 'qtype_code'), $qtype->attachment_options());
        $mform->setDefault('attachments', $this->get_default_value('attachments', 0));

        $mform->addElement('select', 'attachmentsrequired',
                get_string('attachmentsrequired', 'qtype_code'), $qtype->attachments_required_options());
        $mform->setDefault('attachmentsrequired', $this->get_default_value('attachmentsrequired', 0));
        $mform->addHelpButton('attachmentsrequired', 'attachmentsrequired', 'qtype_code');
        $mform->hideIf('attachmentsrequired', 'attachments', 'eq', 0);

        $mform->addElement('filetypes', 'filetypeslist', get_string('acceptedfiletypes', 'qtype_code'));
        $mform->addHelpButton('filetypeslist', 'acceptedfiletypes', 'qtype_code');
        $mform->hideIf('filetypeslist', 'attachments', 'eq', 0);

        $mform->addElement('select', 'maxbytes', get_string('maxbytes', 'qtype_code'), $qtype->max_file_size_options());
        ...
        $mform->addElement('codeeditor', 'responsetemplate', get_string('responsetemplate', 'qtype_code'),
                array('rows' => 10),  array_merge($this->editoroptions, array('maxfiles' => 0, 'language' => $codelanguage)));
        $mform->addHelpButton('responsetemplate', 'responsetemplate', 'qtype_code');

        $mform->addElement('header', 'graderinfoheader', get_string('graderinfoheader', 'qtype_code'));
        $mform->setExpanded('graderinfoheader');
        $mform->addElement('editor', 'graderinfo', get_string('graderinfo', 'qtype_code'),
    ...
    public function validation($fromform, $files) {
        ...
        if ($fromform['responseformat'] == 'noinline' && !$fromform['attachments']) {
            $errors['attachments'] = get_string('mustattach', 'qtype_code');
        }
        ...
        if ($fromform['responseformat'] == 'noinline' && !$fromform['attachmentsrequired']) {
            $errors['attachmentsrequired'] = get_string('mustrequire', 'qtype_code');
        }
        ...
        if ($fromform['attachments'] > 0 && $fromform['attachments'] < $fromform['attachmentsrequired'] ) {
            $errors['attachmentsrequired']  = get_string('mustrequirefewer', 'qtype_code');
        }
        ...
        if ($fromform['responserequired']) {
            if (isset($fromform['minwordenabled'])) {
                ...
                if ($fromform['minwordlimit'] < 0) {
                    $errors['mingroup'] = get_string('err_minwordlimitnegative', 'qtype_code');
                }
                if (!$fromform['minwordlimit']) {
                    $errors['mingroup'] = get_string('err_minwordlimit', 'qtype_code');
                }
            }
            if (isset($fromform['maxwordenabled'])) {
                ...
                if ($fromform['maxwordlimit'] < 0) {
                    $errors['maxgroup'] = get_string('err_maxwordlimitnegative', 'qtype_code');
                }
                if (!$fromform['maxwordlimit']) {
                    $errors['maxgroup'] = get_string('err_maxwordlimit', 'qtype_code');
                }
            }
            if (isset($fromform['maxwordenabled']) && isset($fromform['minwordenabled'])) {
                if ($fromform['maxwordlimit'] < $fromform['minwordlimit'] &&
                    ...
                    $errors['maxgroup'] = get_string('err_maxminmismatch', 'qtype_code');
                }
            }
        }
        return $errors;
