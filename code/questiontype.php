...
/**
 * Defines question type for the code question type.
 *
 * @package    qtype
 * @subpackage code
...
class qtype_code extends question_type {
    ...
    public function save_question_options($formdata) {
        ...
        $options->graderinfo = $this->import_or_save_files($formdata->graderinfo,
                $context, 'qtype_code', 'graderinfo', $formdata->id);
    ...
    /**
     * @return array the different coding languages that the question type supports.
     * internal name => human-readable name.
     * we get them from the global supported languages
     */
    public function response_formats() {
        $config = get_config('editor_monaco');
        if (isset($config->supportedlanguages)) {
            return(json_decode($config->supportedlanguages, true));
        }
        return array('javascript' => get_string('javascript', 'qtype_code'));
    }
    ...
    public function response_required_options() {
        return array(
            1 => get_string('responseisrequired', 'qtype_code'),
            0 => get_string('responsenotrequired', 'qtype_code'),
        );
    }
    ...
    public function response_sizes() {
        $choices = [
            2 => get_string('nlines', 'qtype_code', 2),
            3 => get_string('nlines', 'qtype_code', 3),
        ];
        for ($lines = 5; $lines <= 40; $lines += 5) {
            $choices[$lines] = get_string('nlines', 'qtype_code', $lines);
        }
        return $choices;
    }
    ...
    public function attachments_required_options() {
        return array(
            0 => get_string('attachmentsoptional', 'qtype_code'),
            1 => '1',
            2 => '2',
            3 => '3'
        );
    ...
    public function move_files($questionid, $oldcontextid, $newcontextid) {
        ...
        $fs->move_area_files_to_new_context($oldcontextid,
                $newcontextid, 'qtype_code', 'graderinfo', $questionid);
    }

    protected function delete_files($questionid, $contextid) {
        parent::delete_files($questionid, $contextid);
        $fs = get_file_storage();
        $fs->delete_area_files($contextid, 'qtype_code', 'graderinfo', $questionid);
    }
}
