...
class restore_qtype_code_plugin extends restore_qtype_plugin {
    ...
    protected function define_question_plugin_structure() {
        return array(
            new restore_path_element('code', $this->get_pathfor('/code'))
        );
    }
    ...
    public function process_code($data) {
        ...
        if ($questioncreated) {
            ...
            $newitemid = $DB->insert_record('qtype_code_options', $data);
            $this->set_mapping('qtype_code', $oldid, $newitemid);
        }
    }
    ...
    public static function define_decode_contents() {
        return array(
            new restore_decode_content('qtype_code_options', 'graderinfo', 'qtype_code'),
        );
    }

    /**
     * When restoring old data, that does not have the code options information
     * in the XML, supply defaults.
     */
    protected function after_execute_question() {
        global $DB;

        $codeswithoutoptions = $DB->get_records_sql("
                    SELECT q.*
                      FROM {question} q
                      JOIN {backup_ids_temp} bi ON bi.newitemid = q.id
                 LEFT JOIN {qtype_code_options} qeo ON qeo.questionid = q.id
                     WHERE q.qtype = ?
                       AND qeo.id IS NULL
                       AND bi.backupid = ?
                       AND bi.itemname = ?
                ", array('code', $this->get_restoreid(), 'question_created'));

        foreach ($codeswithoutoptions as $q) {
            $defaultoptions = new stdClass();
            $defaultoptions->questionid = $q->id;
            $defaultoptions->responseformat = 'csharp';
            ...
            $DB->insert_record('qtype_code_options', $defaultoptions);
        }
    }
}
