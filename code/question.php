...
class qtype_code_question extends question_with_responses {
    ...
    /**
     * Gets the renderer for this question type
     *
     * @param moodle_page the page we are outputting to.
     * @return qtype_code_format_renderer_base the response-format-specific renderer.
     */
    public function get_format_renderer(moodle_page $page) {
        return $page->get_renderer('qtype_code', 'format_' . 'editor');
    }
    ...
    public function is_complete_response(array $response) {
        ...
        $meetsinlinereq = $hasinlinetext || (!$this->responserequired);
    ...
    public function check_file_access($qa, $options, $component, $filearea, $args, $forcedownload) {
        ...
        } else if ($component == 'qtype_code' && $filearea == 'graderinfo') {
    ...
    private function check_input_word_count($responsestring) {
        ...
        if ($this->maxwordlimit && $count > $this->maxwordlimit) {
            return get_string('maxwordlimitboundary', 'qtype_code',
                    ['limit' => $this->maxwordlimit, 'count' => $count]);
        } else if ($count < $this->minwordlimit) {
            return get_string('minwordlimitboundary', 'qtype_code',
    ...
    public function get_word_count_message_for_review(array $response): string {
        ...
        if ($this->maxwordlimit && $count > $this->maxwordlimit) {
            return get_string('wordcounttoomuch', 'qtype_code',
                    ['limit' => $this->maxwordlimit, 'count' => $count]);
        } else if ($count < $this->minwordlimit) {
            return get_string('wordcounttoofew', 'qtype_code',
                    ['limit' => $this->minwordlimit, 'count' => $count]);
        } else {
            return get_string('wordcount', 'qtype_code', $count);
        }
    }
}
